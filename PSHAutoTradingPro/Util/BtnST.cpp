...
#include "FontManager.h" // 김동현 2018-02-27 폰트 변경하게 수정
...
void DrawGradient( CDC* pDC , TRIVERTEX* p , CRect rect )
{
	GRADIENT_RECT draw_rect = {0 , 1};
	...
	pDC->GradientFill(p , 2 , &draw_rect , 1 , GRADIENT_FILL_RECT_V);
	...
	swap(p[0].Red , p[1].Red);
	swap(p[0].Green , p[1].Green);
	swap(p[0].Blue , p[1].Blue);
...
CButtonST::CButtonST()
{
	...
	//-----------------------------------------------------------
	// 김동현 2018-02-27 그라데이션이 가능한지 테스트
	m_dwGradient = BTNST_GRADIENT_NOT;
	m_bClicked = FALSE;
	ZeroMemory(m_GradientNoramlData , sizeof(TRIVERTEX) * _countof(m_GradientNoramlData));
	ZeroMemory(m_GradientClickData , sizeof(TRIVERTEX) * _countof(m_GradientClickData));
	//-----------------------------------------------------------
...
BOOL CButtonST::OnClicked()
{
	...
	// dh.kim 2018-07-24 14:27:49 : 그라데이션 그릴때 사용할 토글 변수
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	// Draw button border
	if ( !GetGradient() ) // 김동현 2018-02-27 그라데이션이 가능한지 테스트
	...
	if ( GetGradient() )
	{		
		if ( !m_bClicked && (m_dwGradient & BTNST_GRADIENT_UP) )		DrawGradient( pDC , m_GradientNoramlData , lpDIS->rcItem );
		else if ( m_bClicked && (m_dwGradient & BTNST_GRADIENT_DOWN) )	DrawGradient( pDC , m_GradientClickData , lpDIS->rcItem );

		if ( !sTitle.IsEmpty() )
			DrawTheText(pDC , (LPCTSTR)sTitle , &lpDIS->rcItem , &itemRect , m_bIsPressed , m_bIsDisabled);

		if ( m_bIsPressed )
			pDC->Draw3dRect(&lpDIS->rcItem , ::GetSysColor(COLOR_BTNSHADOW) , ::GetSysColor(COLOR_BTNHILIGHT));
		else
			pDC->Draw3dRect(&lpDIS->rcItem , ::GetSysColor(COLOR_BTNHILIGHT) , ::GetSysColor(COLOR_BTNSHADOW));
	}
	//-----------------------------------------------------------
} // End of DrawItem
...
DWORD CButtonST::SetBitmaps(int nBitmapIn , COLORREF crTransColorIn , int nBitmapOut , COLORREF crTransColorOut)
{
	...
	//---------------------------------------------------------------------------------------
	// 김동현 2018-03-05 함수 수정 ( SetBitmaps 함수 호출하면 자동으로 SizeToContent 호출되게 수정
	// return SetBitmaps(hBitmapIn , crTransColorIn , hBitmapOut , crTransColorOut);
	//---------------------------------------------------------------------------------------
	DWORD dw = SetBitmaps(hBitmapIn , crTransColorIn , hBitmapOut , crTransColorOut);
	if ( BTNST_OK == dw )
		SizeToContent();
...
//------------------------------------------------------------------------------------------------
// 김동현 2018-02-27 기능 수정
DWORD CButtonST::SetFontColor(COLORREF crColor , BOOL bRepaint)
{
	if ( SetColor(BTNST_COLOR_FG_IN , crColor , bRepaint) != BTNST_OK ) return BTNST_INVALIDINDEX;
	if ( SetColor(BTNST_COLOR_FG_OUT , crColor , bRepaint) != BTNST_OK ) return BTNST_INVALIDINDEX;
	if ( SetColor(BTNST_COLOR_FG_FOCUS , crColor , bRepaint) != BTNST_OK ) return BTNST_INVALIDINDEX;

	return BTNST_OK;
}

DWORD CButtonST::SetBkColor(COLORREF crColor , BOOL bRepaint)
{
	if ( SetColor(BTNST_COLOR_BK_IN , crColor , bRepaint) != BTNST_OK ) return BTNST_INVALIDINDEX;
	if ( SetColor(BTNST_COLOR_BK_OUT , crColor , bRepaint) != BTNST_OK ) return BTNST_INVALIDINDEX;
	if ( SetColor(BTNST_COLOR_BK_FOCUS , crColor , bRepaint) != BTNST_OK ) return BTNST_INVALIDINDEX;

	return BTNST_OK;
}

void CButtonST::SetGradient( DWORD ClickMode ,
							 COLORREF NormalBeginColor , 
							 COLORREF NormalEndColor ,
							 COLORREF ClickBeginColor, 
							 COLORREF ClickEndColor )
{
	m_dwGradient = ClickMode;

	m_GradientNoramlData[0].Red = GetRValue(NormalBeginColor) << 8;
	m_GradientNoramlData[0].Green = GetGValue(NormalBeginColor) << 8;
	m_GradientNoramlData[0].Blue = GetBValue(NormalBeginColor) << 8;

	m_GradientNoramlData[1].Red = GetRValue(NormalEndColor) << 8;
	m_GradientNoramlData[1].Green = GetGValue(NormalEndColor) << 8;
	m_GradientNoramlData[1].Blue = GetBValue(NormalEndColor) << 8;

	m_GradientClickData[0].Red = GetRValue(ClickBeginColor) << 8;
	m_GradientClickData[0].Green = GetGValue(ClickBeginColor) << 8;
	m_GradientClickData[0].Blue = GetBValue(ClickBeginColor) << 8;

	m_GradientClickData[1].Red = GetRValue(ClickEndColor) << 8;
	m_GradientClickData[1].Green = GetGValue(ClickEndColor) << 8;
	m_GradientClickData[1].Blue = GetBValue(ClickEndColor) << 8;
}

BOOL CButtonST::SetFontSize(int Size)
{
...
void CButtonST::SetDefaultStyle( int FontSize )
{
	SetGradient( BTNST_GRADIENT_ALL , RGB(70 , 140 , 180) , RGB(50 , 90 , 170) , RGB(70 , 140 , 180) , RGB(50 , 90 , 170) );
	SetFontSize(FontSize);
	SetFontColor(RGB(255 , 255 , 255));
	SetBkColor(RGB(70 , 140 , 180));
}
