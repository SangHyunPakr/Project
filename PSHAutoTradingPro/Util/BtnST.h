...
// dh.kim 2018-07-24 14:07:43 : 그라데이션 클릭 모드
#define BTNST_GRADIENT_NOT  0
#define BTNST_GRADIENT_UP   1
#define BTNST_GRADIENT_DOWN 2
#define BTNST_GRADIENT_ALL  ( BTNST_GRADIENT_UP | BTNST_GRADIENT_DOWN )
...
public:
	...
	DWORD SetFontColor(COLORREF crColor , BOOL bRepaint = TRUE);
	DWORD SetBkColor(COLORREF crColor , BOOL bRepaint = TRUE);
	...
	void SetGradient( DWORD ClickMode , 
					 COLORREF NormalBeginColor = RGB(0,0,0) , 
					 COLORREF NormalEndColor = RGB(0,0,0)   ,
					 COLORREF ClickBeginColor = RGB(0,0,0) , 
					 COLORREF ClickEndColor = RGB(0,0,0)   );
	inline BOOL GetGradient() const { return ( m_dwGradient != BTNST_GRADIENT_NOT ); }
	//-----------------------------------------------------------

	//-----------------------------------------------------------
	// 김동현 2018-02-27 폰트 변경하게 수정
	BOOL SetFontSize(int Size);
	//-----------------------------------------------------------

	// dh.kim 2018-05-10 11:34:43 : 다이얼로그 초기화중에 Button 컨트롤 초기화하는 코드가 중복을 막기 위해서.
	void SetDefaultStyle( int FontSize );
...
protected:
	...
	//-----------------------------------------------------------
	// 김동현 2018-02-27 그라데이션이 가능한지 테스트
	DWORD	  m_dwGradient;				// 그라데이션 모드
	BOOL	  m_bClicked;
	TRIVERTEX m_GradientNoramlData[2];	// 그라데이션 시작 - 끝에 대한 데이터를 저장하는 색 데이터
	TRIVERTEX m_GradientClickData[2];
	//-----------------------------------------------------------
